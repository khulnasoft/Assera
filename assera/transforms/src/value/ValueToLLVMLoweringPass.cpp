...
using namespace assera::transforms::value;
...
enum class ClockID
{
    ASSERA_CLOCK_REALTIME = 0,
    ASSERA_CLOCK_MONOTONIC = 1,
};
...
public:
    ...
    {
        auto* context = module.getContext();
        return getOrInsertLibraryFunction(rewriter, assera::ir::GetPrintErrorFunctionName(), getPrintfType(context), module, llvmDialect);
    }
...
using ValueCallOp = assera::ir::value::CallOp;
...
struct GetTimeOpLowering : public ValueLLVMOpConversionPattern<GetTimeOp>
{
    ...
    GetTimeOpLowering(LLVMTypeConverter& converter, mlir::MLIRContext* context, assera::value::TargetDevice deviceInfo) :
...
{
    ValueToLLVMLoweringPass(bool useBarePtrCallConv, bool emitCWrappers, unsigned indexBitwidth, bool useAlignedAlloc, llvm::DataLayout dataLayout, assera::value::TargetDevice deviceInfo = {}, const IntraPassSnapshotOptions& snapshotteroptions = {}) :
...
mlir::Value GetTimeOpLowering::GetTime(ConversionPatternRewriter& rewriter, GetTimeOp& op, ModuleOp& parentModule) const
{
    ...
    else
    {
        if (isEnterRegionTimer) 
        {
            ...
            Value clockId = rewriter.create<LLVM::ConstantOp>(loc, clockIdTy, rewriter.getI64IntegerAttr(int64_t(ClockID::ASSERA_CLOCK_REALTIME)));
...
void ValueToLLVMLoweringPass::runOnModule()
{
    ...
    target.addLegalDialect<intrinsics::AsseraIntrinsicsDialect>();
...
const mlir::LowerToLLVMOptions& GetDefaultAsseraLLVMOptions(mlir::MLIRContext* context)
{
    ...
    // set Assera alterations to the defaults
    options.allocLowering = mlir::LowerToLLVMOptions::AllocLowering::AlignedAlloc;
...
{
    // The values here should always match the ones specified by GetDefaultAsseraLLVMOptions
