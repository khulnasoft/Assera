...
#ifndef RC_FILE_LOC
#define RC_FILE_LOC(rewriter) assera::ir::util::GetLocation(rewriter, __FILE__, __LINE__)
#endif // RC_FILE_LOC
...
namespace vtr = assera::transforms::value;

namespace assera::generated
{
using llvm::ArrayRef;
...
namespace irutil = assera::ir::util;
...
#define CAST_FROM_TO_WITH_OP(testFromType, testToType, castOp) CAST_FROM_TO_WITH_OP_IF(testFromType, testToType, castOp, true);
                                  ...
                                  PatternRewriter& rewriter) const final
    {
        ...
        auto signlessFromValue = assera::ir::util::ToSignlessMLIRValue(rewriter, op.source());
        auto signlessToType = assera::ir::util::ToSignlessMLIRType(rewriter, toType);
        ...
        auto i64IntermediateType = assera::ir::util::CloneTypeWithNewElementType(op.source().getType(), rewriter.getI64Type());
    ...
    PatternRewriter& rewriter) const
{
    ...
    auto result = [&]() -> mlir::Value {
        using assera::ir::value::BinaryOpPredicate;
    ...
    PatternRewriter& rewriter) const
{
    ...
    if (auto inputShape = inputType.dyn_cast<ShapedType>())
    {
        if (inputShape.hasStaticShape() && inputShape.getNumElements() == 1)
        {
            ...
            auto loadedElt = rewriter.create<assera::ir::value::GetElementOp>(loc, input);
            rewriter.create<assera::ir::value::StoreOp>(loc, loadedElt, output, indices);
        }
        else
        {
...
{
    mlir::MLIRContext* context = patterns.getContext();
    assera::generated::populateWithGenerated(patterns);
    patterns.insert<
