...
using namespace assera::ir::executionPlan;
...
mlir::Value CreateProductOfValues(OpBuilder& builder, Location loc, Type elementType, ValueRange values)
{
    ...
    mlir::Value currentProduct = builder.create<assera::ir::value::CastOp>(loc, constantOne, elementType, true /* compiler-internal */);
...
struct LoopnestInfo
{
    ...
    std::vector<assera::ir::loopnest::Range> fullySplitRanges;
                                              ...
                                              const std::vector<int64_t>& domainDimSizes)
{
    ...
    {
        assera::ir::loopnest::Range dimRange(0, domainDimSize);
        indexRanges.emplace_back(domainDimName, dimRange);
    }
    return { indexRanges };
}

void SetExecTarget(mlir::OpBuilder& builder, NestOp& nestOp, ExecPlanOp& execPlanOp, const ExecutionTarget& execTarget, const std::optional<assera::ir::targets::GPU>& gpuLaunchOptions)
{
...
std::tuple<NestOp, ScheduleOp, ExecPlanOp> CreateCacheLoopnestHelper(
    ...
    const std::optional<assera::ir::targets::GPU>& gpuLaunchOptions,
    ...
    const std::function<void(OpBuilder&, const std::vector<mlir::Value>&, const std::vector<mlir::Value>&)>& kernelFn)
{
    ...
    std::transform(activeBlockShape.begin(), activeBlockShape.end(), std::back_inserter(loopnestInfo.fullySplitRanges), [&](int64_t indexRange) -> assera::ir::loopnest::Range {
        return assera::ir::loopnest::Range(0, indexRange, 1);
    });
...
ActiveBlockInfo ConvertMemRefRegionToActiveBlockInfo(OpBuilder& builder, const mlir::MemRefRegion& activeBlockRegion)
{
    ...
    if (!IsStaticallySized(activeBlockRegion))
    {
        throw assera::utilities::LogicException(assera::utilities::LogicExceptionErrors::notImplemented, "Dynamically-sized cache active blocks are not currently supported");
    }
                                                 ...
                                                 const std::vector<mlir::Value>& multiCacheExternalSymbols)
{
    ...
    {
        assera::ir::loopnest::Range dimRange(0, activeBlockInfo.shape[activeBlockDim]);
                    ...
                    const std::vector<mlir::Value>& baseArrayPosition)
{
    ...
    {
        storeValue = builder.create<assera::ir::value::CastOp>(loc, value, dstElementType, true /* compiler-internal */);
    }
...
MultiCacheLoopInfo CreateMultiCacheLoops(mlir::OpBuilder& builder, MultiCacheCopyOp copyOp, const std::function<void(mlir::OpBuilder&, const MultiCacheLoopInfo&)>& fn)
{
    ...
    for (unsigned multiCacheDim = 0; multiCacheDim < multiCacheLoopCount; ++multiCacheDim)
    {
        ...
        assert(constantTripCountOpt.hasValue() && "AffineForOps in Assera loop nests must have constant trip counts");
...
void CreateParametricIVBoundWrapperLoopnest(mlir::OpBuilder& builder,
                                            ...
                                            FnType&& fn)
{
    // The base dimensions of an assera loopnest have unit step size, so construct a loopnest
    ...
    if (!dimsToBind)
    {
        ...
        assera::ir::loopnest::Range range(0, 1);
    ...
    else
    {
        for (const auto& [index, deviceDimBoundIV] : arrayAccessInfo.parametricIVHandles)
        {
            ...
            assera::ir::loopnest::Range range(lb, ub, step);
            nestIndexRanges.emplace_back(index, range);
...
LogicalResult CacheFillOpRewrite::matchAndRewrite(CacheFillOp cacheFillOp, PatternRewriter& rewriter) const
{
    ...
    auto castVal = currentBuilder.create<assera::ir::value::CastOp>(loc, cacheFillOp.value(), innerElementType, true /* compiler-internal */);
...
LogicalResult ActiveElementCacheCopyOpRewrite::matchAndRewrite(ActiveElementCacheCopyOp cacheCopyOp, PatternRewriter& rewriter) const
{
    ...
    if (execTarget == v::ExecutionTarget::GPU && !IsMemspaceLocal(dstMemRefSpace))
    {
        ...
        auto gpuParams = assera::ir::targets::GPU::FromArrayAttr(launchAttr);
...
LogicalResult ActiveBlockCacheReduceOpRewrite::matchAndRewrite(ActiveBlockCacheReduceOp cacheReduceOp, PatternRewriter& rewriter) const
{
    ...
    else
    {
        ...
        if (constantShapeOpt.has_value())
        {
            ...
            auto launchAttr = vLambdaOp->getAttrOfType<mlir::ArrayAttr>(vLambdaOp.getGPULaunchAttrName());
            std::optional<assera::ir::targets::GPU> gpuParams;
            if (launchAttr != nullptr)
            {
                gpuParams = assera::ir::targets::GPU::FromArrayAttr(launchAttr);
            }
...
LogicalResult BeginCreateCacheOpRewrite::matchAndRewrite(BeginCreateCacheOp beginCreateCacheOp, PatternRewriter& rewriter) const
{
    ...
    for (auto& cacheLevelLoop : cacheLevelLoops)
    {
        ...
        if (beginCreateCacheOp.activeBlockCache())
        {
            ...
            // Since Assera deals in statically sizd loopnests, the MemRefRegion::getConstantBoundingSizeAndShape utility will
            ...
            // Since Assera loopnests operate with fixed-size loops, we will assume that the multiCache loops
...
LogicalResult MaxElementCacheRegionOpRewrite::matchAndRewrite(BeginCreateMaxElementCacheOp beginCreateMaxElementCacheOp, PatternRewriter& rewriter) const
{
    ...
    auto doubleBufferMemorySpace = assera::ir::value::MemorySpace::None;
...
LogicalResult TensorizeAffineForOpConversion::matchAndRewrite(AffineForOp affineForOp, PatternRewriter& rewriter) const
{
    ...
    // TODO: Figure out if there's a better way to list the possible OPs that can be used to change type but still be valid IR (maybe an Assera specific cast OP that takes an attr instead?)
    while (innerLoopBodyIter != innerLoopBodyEnd && isa<assera::ir::value::CastOp,
...
void ExecutionPlanCacheRegionLoweringPass::runOnOperation()
{
    ...
    assera::transforms::executionPlan::populateExecutionPlanCacheRegionPatterns(patterns);
...
void ExecutionPlanParallelizationPass::runOnOperation()
{
    ...
    RewritePatternSet patterns(&getContext());
    assera::transforms::executionPlan::populateExecutionPlanParallelizePatterns(patterns);
...
void ExecutionPlanTensorizationPass::runOnOperation()
{
    ...
    assera::transforms::executionPlan::populateExecutionPlanTensorizePatterns(patterns);
...
void ExecutionPlanMakeCacheLoweringPass::runOnOperation()
{
    ...
    RewritePatternSet patterns(&getContext());
    assera::transforms::executionPlan::populateExecutionPlanCacheFinalizePatterns(patterns);
...
void ExecutionPlanCopyReduceLoweringPass::runOnOperation()
{
    ...
    RewritePatternSet patterns(&getContext());
    assera::transforms::executionPlan::populateExecutionPlanCopyReducePatterns(patterns);
...
void ExecutionPlanScaleHoistingPass::runOnOperation()
{
    ...
    RewritePatternSet patterns(&getContext());
    assera::transforms::executionPlan::populateExecutionPlanScaleHoistingPatterns(patterns);
...
void OutOfBoundsAccessHandlingPass::runOnOperation()
{
    ...
    RewritePatternSet patterns(&getContext());
    assera::transforms::affine::populateBoundsCheckingPatterns(patterns);

    if (failed(applyPartialConversion(getOperation(), target, std::move(patterns))))
        signalPassFailure();
}

namespace assera::transforms::executionPlan
{
