...
class VulkanLaunchFuncToVulkanCallsWithTimingPass
    : public assera::transforms::ConvertVulkanLaunchFuncToVulkanCallsWithTimingBase<VulkanLaunchFuncToVulkanCallsWithTimingPass>
{
private:
    ...
    /// Checks whether the given LLVM::LLVMFuncOp is marked as an Assera GPU
    /// initialization func
    bool isRCGPUInitializeFunc(LLVM::LLVMFuncOp op)
    {
        auto funcTags = op->getAttrOfType<DictionaryAttr>(assera::ir::FunctionTagsAttrName);
        if (funcTags == nullptr)
        {
            return false;
        }
