...
namespace value
{
    ...
    public:
        ...
        GetT<std::add_const_t<T>> Get() const
        {
            ...
            if (!ptr.has_value())
            {
                throw utilities::LogicException(utilities::LogicExceptionErrors::notImplemented,
                                                assera::utilities::FormatString("Cannot get const value of type %s from Value of type %s", typeid(T).name(), ToString(GetBaseType()).c_str()));
            }
        ...
        GetT<T> Get()
        {
            ...
            {
                throw utilities::LogicException(utilities::LogicExceptionErrors::notImplemented,
                                                assera::utilities::FormatString("Cannot get value of type %s from Value of type %s", typeid(T).name(), ToString(GetBaseType()).c_str()));
            }
...
template <>
struct hash<::assera::value::Value>
{
    using Type = ::assera::value::Value;

    [[nodiscard]] size_t operator()(const Type& value) const noexcept;
};

template <>
struct equal_to<::assera::value::Value>
{
    using Type = ::assera::value::Value;
