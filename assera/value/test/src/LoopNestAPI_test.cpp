...
Scalar GotoBLASGemm_HighLevelAPI()
{
    using dtype = double;
    int M = 16;
    int N = 16;
    int K = 16;
    auto A = MakeMatrix<dtype>(M, K, "A");
    auto B = MakeMatrix<dtype>(K, N, "B");
    auto C = MakeMatrix<dtype>(M, N, "C");

    // fill with 1's
    Scalar counter = Cast<dtype>(1.0);
    ForRange(M, [&](Scalar m) {
        ForRange(K, [&](Scalar k) {
            A(m, k) = counter;
            counter += Cast<dtype>(0.1);
        });
    });
    counter = Cast<dtype>(1.0);
    ForRange(K, [&](Scalar k) {
        ForRange(N, [&](Scalar n) {
            B(k, n) = Cast<dtype>(counter);
            counter += Cast<dtype>(0.1);
        });
    });

    auto expected = MakeMatrix<dtype>(M, N, "expected");
    ForRange(M, [&](Scalar m) {
        ForRange(N, [&](Scalar n) {
            ForRange(K, [&](Scalar k) {
    ...
    auto nest = Using({ A, B }, ArgumentType::Input)
                    ...
                    .ForAll(n, 0, N)
                    .ForAll(k, 0, K)
                    .Do([](Matrix A, Matrix B, Matrix C, Scalar m, Scalar n, Scalar k) {
                        C(m, n) += A(m, k) * B(k, n);
                    });
    ...
    int cacheStripeSizeM = 4;
    int cacheStripeSizeN = 4;

    int cacheSizeM = 8;
    int cacheSizeK = 8;
    int cacheSizeN = 8;

    int cacheBlockSizeM = 2 * cacheStripeSizeM;
    int cacheBlockSizeK = 4;
    int cacheBlockSizeN = cacheStripeSizeN;

    auto& schedule = nest.GetSchedule();
    auto mCache = schedule.Split(m, cacheSizeM);
    auto mBlock = schedule.Split(m, cacheBlockSizeM);
    auto mCacheStripe = schedule.Split(m, cacheStripeSizeM);
    auto mKernel = schedule.Split(m, kernelSizeM);
    auto kCache = schedule.Split(k, cacheSizeK);
    auto kBlock = schedule.Split(k, cacheBlockSizeK);
    auto nCache = schedule.Split(n, cacheSizeN);
    auto nBlock = schedule.Split(n, cacheBlockSizeN);
    auto nKernel = schedule.Split(n, kernelSizeN);

    std::tuple<bool, int, int, Index> vectorizedCacheExtras;
    vectorizedCacheExtras = { false, kernelSizeM, kernelSizeN, mKernel };

    std::tuple<int, Index, int, Index, BoundaryConditionHandling> progressiveBlasNCopyExtras = { cacheBlockSizeM, mBlock, cacheStripeSizeM, mCacheStripe, BoundaryConditionHandling::ZeroPadding };
    schedule.Cache<ProgressiveBLASNCopy>(A,
                                         { m, k },
                                         { cacheSizeM, cacheSizeK },
                                         { mCache, kCache },
                                         std::nullopt, // Order isn't used by ProgressiveBLASNCopy
                                         progressiveBlasNCopyExtras);

    std::tuple<int, Index, BoundaryConditionHandling> blasTCopyExtras = { cacheStripeSizeN, nBlock, BoundaryConditionHandling::ZeroPadding };
    schedule.Cache<BLASTCopy>(B,
                              { k, n },
                              { cacheSizeK, cacheSizeN },
                              { kCache, nCache },
                              std::nullopt, // Order isn't used by BLASTCopy
                              blasTCopyExtras);

    schedule.Cache<VectorizedOutputCachingStrategy>(C,
                                                    { m, n },
                                                    { cacheStripeSizeM, cacheBlockSizeN },
                                                    { mCacheStripe, nBlock },
                                                    utilities::RowMajorMatrixOrder,
                                                    vectorizedCacheExtras);

    schedule.SetOrder({ mCache, kCache, nCache, mBlock, mCacheStripe, nBlock, kBlock, k, mKernel, m, nKernel, n });
    schedule.Unroll(nKernel);
    schedule.Unroll(mKernel);
    schedule.Unroll(m);
    schedule.Unroll(n);
    schedule.Unroll(k);
        ...
        (void)DeclareFunction("LoopNext_tunable_" + engine.ToString())
            ...
            .Define([&](Matrix matrix) {
                ...
                auto nest = Using({ matrix }, ArgumentType::InputOutput)
                                ...
                                .ForAll(j, 0, static_cast<int>(output.Columns()))
                                .Do([](Matrix m, Scalar i, Scalar j) {
                                    Scalar v = Allocate(m.GetType(), ScalarLayout);
