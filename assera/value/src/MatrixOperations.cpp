...
namespace value
{
    ...
    Matrix ToMatrix(Value data, int numRows, int numCols)
    {
        ...
        if (size != numRows * numCols || !data.GetLayout().IsContiguous())
        {
            throw InputException(InputExceptionErrors::invalidArgument,
                                 assera::utilities::FormatString("data must be contiguous and have size %zu = %d * %d", size, numRows, numCols));
        }
    ...
    Vector MatrixVectorMultiply(Matrix m, Vector v)
    {
        ...
        {
            throw InputException(InputExceptionErrors::invalidArgument,
                                 assera::utilities::FormatString("Vector size %d must match number of columns in the matrix %d", v.Size(), m.Columns()));
        }
        first = 1;
