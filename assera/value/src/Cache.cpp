...
namespace value
{
    ...
    protected:
        CacheImpl(ScheduleOp schedule, std::variant<Value, CacheImpl*> input, CacheIndexing cacheIndexMapping) :
            ...
            _cacheId(assera::ir::util::GetUniqueId(_scheduleOp))
        {
            ...
            else
            {
                ...
                {
                    throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Source cache for hierarchical cache is invalid becaase it doesn't have a cache mlir::Value");
                }
            }
        }
    ...
    protected:
        ...
        {
            _vModuleOp = assera::ir::util::CastOrGetParentOfType<vir::ValueModuleOp>(_scheduleOp);
        }
        ...
        {
            return assera::ir::util::CastOrGetParentOfType<vir::ValueFuncOp>(_scheduleOp);
        }
    ...
    private:
        void AddCacheZero(mlir::OpBuilder& builder, mlir::Value cache)
        {
            ...
            auto valType = assera::ir::util::ToSignlessMLIRType(builder, innerElementType);
        ...
        void CreatePackingFunction(mlir::OpBuilder& builder, const std::string& packingFunctionName)
        {
            ...
            auto loc = GetLocation();
            auto insertionPoint = assera::ir::util::GetTerminalInsertPoint<vir::ValueModuleOp, vir::ModuleTerminatorOp>(_vModuleOp);
    ...
    public:
            ...
            OfflineCacheImpl(schedule, value, mapping)
        {
            ...
            case ValueType::Double:
                ...
                break;
            default:
                throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Unsupported element type for caching");
            }
            ...
            // Create a wrapper function that calls the assera function with the schedule op but hard-codes the arg corresponding
    ...
    private:
        ...
        template <typename ElementType>
        assera::value::Array EmbedPackedBuffer(mlir::OpBuilder& builder, mlir::Value constData, const std::string& packedBufferName)
        {
            auto packedLayout = assera::utilities::MemoryLayout{ assera::utilities::MemoryShape{ GetCacheShape() } };

            auto constantDataGlobalRef = mlir::dyn_cast_or_null<assera::ir::value::ReferenceGlobalOp>(constData.getDefiningOp());
            ...
            assera::value::Array packedBuffer;
            {
                ...
                auto insertionPoint = assera::ir::util::GetTerminalInsertPoint<vir::ValueModuleOp, vir::ModuleTerminatorOp>(_vModuleOp);
                builder.restoreInsertionPoint(insertionPoint);
                packedBuffer = assera::value::Array(packedBufferInitData, packedLayout, packedBufferName);
            }
            return packedBuffer;
        }
        ...
        void CreateWrapperFunction(mlir::OpBuilder& builder, const std::string& wrapperFnName)
        {
            ...
            auto globalScopeGlobalRef = mlir::dyn_cast_or_null<assera::ir::value::ReferenceGlobalOp>(_packedBufferMlirValue.getDefiningOp());
            auto localScopeGlobalRef = builder.create<assera::ir::value::ReferenceGlobalOp>(GetLocation(), globalScopeGlobalRef.getGlobal());
        ...
        assera::value::Array _packedBuffer;
