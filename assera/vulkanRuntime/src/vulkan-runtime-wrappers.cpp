...
// Ported from <llvm-project>/mlir/tools/mlir-vulkan-runner/vulkan-runtime-wrappers.cpp
...
public:
    ...
    {
        if (failed(vulkanRuntime.initRuntime()))
        {
            std::cerr << "Vulkan Runtime initialization failed";
        }
    }
    ...
    ~VulkanRuntimeManager()
    {
        ...
        {
            std::cerr << "Vulkan Runtime destruction failed";
        }
    }

    void setRepeatedRunCharacteristics(uint32_t printTimings, uint32_t warmupCount, uint32_t runCount)
    {
        std::lock_guard<std::mutex> lock(mutex);
        vulkanRuntime.setRepeatedRunCharacteristics(printTimings == 1, warmupCount, runCount);
    }

    void setResourceData(DescriptorSetIndex setIndex, BindingIndex bindIndex, const VulkanHostMemoryBuffer& memBuffer)
    {
        std::lock_guard<std::mutex> lock(mutex);
...
void bindMemRef(void* vkRuntimeManager, DescriptorSetIndex setIndex, BindingIndex bindIndex, MemRefDescriptor<T, S>* ptr)
{
...
void setRepeatedRunCharacteristics(void* vkRuntimeManager, uint32_t printTimings, uint32_t warmupCount, uint32_t runCount)
{
    reinterpret_cast<VulkanRuntimeManager*>(vkRuntimeManager)
        ->setRepeatedRunCharacteristics(printTimings, warmupCount, runCount);
}
...
#define DECLARE_BIND_MEMREF(size, type, typeName)                                                                       \
    VULKAN_WRAPPER_SYMBOL_EXPORT void bindMemRef##size##D##typeName(                                                    \
        void* vkRuntimeManager, DescriptorSetIndex setIndex, BindingIndex bindIndex, MemRefDescriptor<type, size>* ptr) \
    {                                                                                                                   \
...
_mlir_ciface_fillResource3DFloat(MemRefDescriptor<float, 3>* ptr, // NOLINT
                                 float value)
{
    std::fill_n(ptr->allocated, ptr->sizes[0] * ptr->sizes[1] * ptr->sizes[2], value);
}
