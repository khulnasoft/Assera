...
namespace loopnest
{
    ...
    Index TransformedDomain::Pad(const Index& index, int64_t size, bool padFront, mlir::MLIRContext* context)
    {
        ...
        if (_indices.count(index) == 0)
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Padding an unknown index");
        if (!IsLoopIndex(index))
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Can't pad already-transformed index");

        auto parentRange = _indices[index].range;
        if (!parentRange.HasConstantEnd())
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Can't pad non-constant index ranges");
        if (parentRange.Increment() != 1)
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Can't pad index ranges with increments != 1");
    ...
    SplitIndex TransformedDomain::Split(const Index& index, int64_t splitSize, mlir::MLIRContext* context)
    {
        ...
        if (_indices.count(index) == 0)
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Splitting an unknown index");
        if (!IsLoopIndex(index))
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Cannot split an already-transformed index");
    ...
    Index TransformedDomain::Skew(const Index& index, const Index& referenceIndex, mlir::MLIRContext* context)
    {
        ...
        if (_indices.count(index) == 0 || _indices.count(referenceIndex) == 0)
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Skewing or referencing an unknown index");
        if (!IsLoopIndex(index) || !IsLoopIndex(referenceIndex))
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Can't skew or reference an already-transformed index");

        auto parentRange = _indices[index].range;
        auto referenceRange = _indices[referenceIndex].range;
        if (!parentRange.HasConstantEnd() || !referenceRange.HasConstantEnd())
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Can't skew non-constant index ranges");
        if (parentRange.Increment() != 1 || referenceRange.Increment() != 1)
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Can't skew index ranges with increments != 1");

        if (!_indices[index].padding.empty())
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Can't skew padded indices");
    ...
    bool TransformedDomain::HasConstantDimensionSize(const Index& dimensionIndex) const
    {
        if (!IsDimension(dimensionIndex))
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "unknown dimension");
    ...
    {
        if (_indices.count(index) == 0)
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "unknown index");
    ...
    {
        if (!IsDimension(dimensionIndex))
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "index isn't a dimension");
    ...
    Index TransformedDomain::GetOtherSplitIndex(const Index& index) const
    {
        ...
        throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "GetOtherSplitIndex() --- called on a non-split index");
    }
    ...
    Index TransformedDomain::GetBaseIndex(const Index& index) const
    {
        ...
        if (baseIndices.size() != 1)
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Getting base index of an index with multiple parents");
    ...
    TransformedDomain TransformedDomain::Fuse(const std::vector<TransformedDomain>& domains, const std::vector<std::vector<Index>>& indexCorrespondences)
    {
        ...
        {
            throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Index correspondences don't match number of domains");
        }
        ...
        auto updateFusedPadding = [](const TransformedDomain& domain, const Index& index, std::vector<int64_t>& fusedPadding) -> void {
            ...
            else if (padding.size() > 1)
            {
                ...
                throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Fusing an already-fused padded index is not supported.");
            }
            else
            {
        ...
        for (const auto& correspondingIndices : indexCorrespondences)
        {
            ...
            for (unsigned idx = 1; idx < correspondingIndices.size(); ++idx)
            {
                ...
                {
                    throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Fused index ranges do not match. The smaller index needs to be padded.");
                }
                ...
                if (domains[idx].IsPaddedIndex(i2))
                {
                    ...
                    {
                        throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument, "Fusing an index based on multiple dimensions is not supported.");
                    }
    ...
    bool operator==(const TransformedDomain& a, const TransformedDomain& b)
    {
        ...
        auto dimRange = assera::utilities::MakeZipRange(aDimensions, bDimensions);
        ...
        auto indicesRange = assera::utilities::MakeZipRange(aIndices, bIndices);
