...
struct MergeSliceOps : public mlir::OpRewritePattern<v::SliceOp>
{
    ...
    void rewrite(v::SliceOp op, mlir::PatternRewriter& rewriter) const override
    {
        ...
        auto incomingDims = assera::ir::util::ConvertArrayAttrToIntVector(incoming.sliceDimensions());
        auto incomingDimsAndOffsets = llvm::zip(incomingDims, incoming.offsets());
        auto thisDims = assera::ir::util::ConvertArrayAttrToIntVector(adaptor.sliceDimensions());
...
struct ValueBinOpSimplification : public mlir::OpRewritePattern<v::BinOp>
{
    ...
    mlir::LogicalResult rewriteConstantBinOp(v::BinOp op, mlir::PatternRewriter& rewriter) const
    {
        ...
        auto resultElementType = assera::ir::util::GetElementType(op.result().getType());
        auto lhsElementType = assera::ir::util::GetElementType(lhs.getType());
        auto rhsElementType = assera::ir::util::GetElementType(rhs.getType());
