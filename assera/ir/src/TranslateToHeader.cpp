...
namespace ir
{
    namespace
    {
        ...
        std::string GetHeaderPrologue(const std::string& libraryName)
        {
            ...
            os << "//\n";
            os << "// Header for Assera library " << libraryName << "\n";
            os << "//\n\n";
            ...
            // for float16_t
            os << "#if !defined(ASSERA_FLOAT)\n";
            os << "#define ASSERA_FLOAT 1\n";
            os << "typedef uint16_t float16_t;\n";
            os << "typedef uint16_t bfloat16_t;\n";
            os << "#endif // !defined(ASSERA_FLOAT)\n";
        ...
        std::string GetDebugPrologue()
        {
            ...
            os << "#ifndef _ASSERA_SYMBOL_EXPORT\n";
            os << "#ifdef _WIN32\n";
            os << "#define _ASSERA_SYMBOL_EXPORT __declspec(dllexport)\n";
            os << "#else\n";
            os << "#define _ASSERA_SYMBOL_EXPORT __attribute__((visibility(\"default\")))\n";
            os << "#endif // _WIN32\n";
            os << "#endif // _ASSERA_SYMBOL_EXPORT\n\n";
        ...
        std::string GetDebugCode()
        {
            ...
            os << "_ASSERA_SYMBOL_EXPORT int " << fnName << "(const char* fmt, ...) {\n";
            os << "    va_list args;\n";
        ...
        bool DebugMode(std::vector<value::ValueModuleOp> valueModuleOps)
        {
            return std::any_of(valueModuleOps.begin(), valueModuleOps.end(), [](auto m) {
                return m->getAttr(assera::ir::GetDebugModeAttrName());
            });
        }
        ...
        std::vector<hat::UsageType> GetHATParameterUsages(const value::ValueFuncOp& funcOp)
        {
            ...
            {
                auto usagesVec = assera::ir::util::ConvertArrayAttrToIntVector(usagesAttr);
                std::transform(usagesVec.cbegin(), usagesVec.cend(), std::back_inserter(hatUsages), [](int usage) {
                    switch (static_cast<assera::value::FunctionParameterUsage>(usage))
                    {
                    case assera::value::FunctionParameterUsage::input:
                        return hat::UsageType::Input;
                    case assera::value::FunctionParameterUsage::output:
                        return hat::UsageType::Output;
                    case assera::value::FunctionParameterUsage::inputOutput:
                    default:
        ...
        mlir::LogicalResult WriteFunctionDeclaration(StreamType& os, value::ValueFuncOp fn, bool useBarePtrCallConv)
        {
            ...
            if (name.find(assera::value::FunctionDeclaration::GetTemporaryFunctionPointerPrefix(), 0) == 0)
            {
...
static mlir::TranslateFromMLIRRegistration
    registration("mlir-to-header", [](mlir::ModuleOp module, llvm::raw_ostream& output) {
        return assera::ir::TranslateToHeader(module, output);
    });
