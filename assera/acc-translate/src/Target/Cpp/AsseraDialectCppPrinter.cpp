...
#include "AsseraDialectCppPrinter.h"
...
{
    LogicalResult AsseraDialectCppPrinter::printOp(vir::CallOp callOp)
    {
    ...
    LogicalResult AsseraDialectCppPrinter::printOp(vir::ReturnOp returnOp)
    {
        os << "return";
    ...
    LogicalResult AsseraDialectCppPrinter::printOp(vir::WarpIdOp warpIdOp)
    {
    ...
    LogicalResult AsseraDialectCppPrinter::printOp(vir::MMAAllocSyncOp allocMatrixOp)
    {
        if (!state.hasRuntime(Runtime::CUDA))
        {
    ...
    // LogicalResult AsseraDialectCppPrinter::printOp(vir::MMAFillSyncOp constantMatrixOp)
    // {
    //     if (!state.hasRuntime(Runtime::CUDA))
    //     {
    ...
    LogicalResult AsseraDialectCppPrinter::printOp(vir::MMALoadSyncOp loadMatrixOp)
    {
        if (!state.hasRuntime(Runtime::CUDA))
        {
    ...
    LogicalResult AsseraDialectCppPrinter::printOp(vir::MMAComputeSyncOp computeMatrixOp)
    {
        if (!state.hasRuntime(Runtime::CUDA))
        {
    ...
    LogicalResult AsseraDialectCppPrinter::printOp(vir::MMAStoreSyncOp storeMatrixOp)
    {
        if (!state.hasRuntime(Runtime::CUDA))
        {
    ...
    LogicalResult AsseraDialectCppPrinter::printVectorType(mlir::Type elementType, const uint32_t stride) const
    {
        if (state.hasRuntime(Runtime::ROCM))
        {
    ...
    LogicalResult AsseraDialectCppPrinter::printOp(vir::GPUBlockCacheOp blockLoadOp)
    {
        if (!state.hasRuntime(Runtime::CUDA))
        {
        ...
        const auto tileShape = assera::ir::util::ConvertArrayAttrToIntVector(blockLoadOp.tileShape());
    ...
    LogicalResult AsseraDialectCppPrinter::printDialectOperation(
        Operation* op,
        bool* /*skipped*/,
        bool* consumed)
    {
    ...
    LogicalResult AsseraDialectCppPrinter::printIntrinsicCallOp(Operation* callOp,
                                                                Operation* defFuncOp,
                                                                bool* consumed)
    {
        *consumed = false;
    ...
    LogicalResult AsseraDialectCppPrinter::printPrologue()
    {

        return success();
    }

    LogicalResult AsseraDialectCppPrinter::printEpilogue()
    {
        // TODO: add a cmdline option to skip generating host launch func
        return success();
    }

    LogicalResult AsseraDialectCppPrinter::runPrePrintingPasses(Operation* op)
    {
