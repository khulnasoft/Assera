...
for testname in ${tests}; do
    ...
    (
        ${ASSERA_DIR}/build_cmake/bin/acc-opt --verify-each=false --optimize-barriers build/_tmp/${testname}/11_BarrierOpt.mlir > ${testname}_post.mlir
        # ${ASSERA_DIR}/build_cmake/bin/acc-opt --verify-each=false --acc-to-llvm="runtime=vulkan target=host enable-profiling=false gpu-only=false barrier-opt-dot barrier-opt-dot-filename=${testname}_graph.dot" --print-ir-after=optimize-barriers --mlir-disable-threading -o /dev/null ${testname}.mlir > /dev/null >& ${testname}_post.mlir
    )
