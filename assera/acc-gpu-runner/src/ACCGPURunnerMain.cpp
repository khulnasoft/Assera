...
#include <transforms/include/gpu/AsseraVulkanPasses.h>
...
namespace
{
static llvm::cl::OptionCategory RCGPURunnerOptions("Assera GPU Runner Options");
static std::vector<const llvm::cl::OptionCategory*> AsseraGPURunnerCategories{
    &RCGPURunnerOptions
};
...
llvm::cl::opt<bool> printRCIR{ "printRCIR",
                               llvm::cl::desc("Print out MLIR after Assera passes"),
                               llvm::cl::init(false),
                               llvm::cl::cat(RCGPURunnerOptions) };
...
void AddMLIRVulkanRunnerPasses(PassManager& passManager)
{
    ...
    passManager.addPass(assera::transforms::createAsseraToSPIRVPass());
    ...
    passManager.addPass(assera::transforms::vulkan::createEmitVulkanWrapperPass());
    ...
    passManager.addPass(assera::transforms::value::createValueToLLVMPass(llvmOptions));
    passManager.addPass(createLowerToLLVMPass(llvmOptions));
    passManager.addPass(assera::transforms::value::createFunctionPointerResolutionPass());
    assera::transforms::vulkan::VulkanTimingOptions vulkanTimingOptions = {
        printTiming,
        warmupCount,
        runCount
    };
    passManager.addPass(assera::transforms::vulkan::createConvertVulkanLaunchFuncToVulkanCallsWithTimingPass(vulkanTimingOptions));
}

void AddAsseraLoweringPasses(PassManager& passManager)
{
    namespace v = assera::ir::value;

    // TODO : converge locations containing the Assera pass pipeline

    auto& valueFuncOpPM = passManager.nest<v::ValueModuleOp>().nest<v::ValueFuncOp>();
    valueFuncOpPM.addPass(createCanonicalizerPass());
    valueFuncOpPM.addPass(assera::transforms::loopnest::createLoopNestToValueFuncPass());
    passManager.addPass(assera::transforms::value::createValueFuncToTargetPass());
    ...
    passManager.addPass(assera::transforms::value::createValueToStdPass());
...
int main(int argc, char** argv)
{
    ...
    llvm::cl::HideUnrelatedOptions(AsseraGPURunnerCategories);

    // explicitly include the pass manager cl options for things like pass statistics or printing IR after passes, etc.
    registerPassManagerCLOptions();

    llvm::cl::ParseCommandLineOptions(argc, argv, "Assera GPU JIT Runner\n");
    ...
    MLIRContext context;
    context.appendDialectRegistry(assera::ir::GetDialectRegistry());
    ...
    AddAsseraLoweringPasses(passManager);
    if (printRCIR)
    {
        passManager.addPass(
