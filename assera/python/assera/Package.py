...
def _emit_module(module_to_emit, target, mode, output_dir, name):
    ...
    proj = accc.AsseraProject(output_dir=working_dir, library_name=name)
...
class Package:
    ...
    def _create_gpu_utility_module(self, compiler_options, target, mode, output_dir, name="AsseraGPUUtilities"):
        gpu_utility_module = _lang_python._Module(name=name, options=compiler_options)

        with SetActiveModule(gpu_utility_module):
            gpu_init_fn = _lang_python._DeclareFunction("AsseraGPUInitialize")
            gpu_deinit_fn = _lang_python._DeclareFunction("AsseraGPUDeInitialize")
    ...
    def _create_mapping_of_heuristic_parameters_with_possible_values(
        self, source: Union["assera.Nest", "assera.Schedule", "assera.Plan", "assera.Function", Callable]
    ):
        parameter_dict = {}
    ...
    def add(
        self,
        source: Union["assera.Nest", "assera.Schedule", "assera.Plan", "assera.Function", Callable],
        args: List[Union["assera.Dimension", "assera.Array"]] = None,
        base_name: str = "",
        parameters: Union[dict, List[dict]] = {},
        function_opts: dict = {},
        auxiliary: dict = {}
    ) -> Union["assera.Function", List["assera.Function"]]:
    ...
    def build(
        ...
        proj = accc.AsseraProject(output_dir=working_dir, library_name=name, output_type=output_type)
