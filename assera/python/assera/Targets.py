...
class Target(_TargetContainer):
    ...
    def _try_get_known_name(self, known_name):
        if known_name == "HOST":
            ...
            print(
                colored(
                    f"""Warning: Your host machine "{cpu_info['brand_raw']}" is not a known target model. To generate optimal code, we recommend that you inspect the assera.Target.Models enumeration to find the closest matching target model,
and create a Target using that model. You may also define a custom target if there is no closest match.
For more details please refer to this link: https://khulnasoft.github.io/Assera/Reference/classes/Target/Target/#known-device-names""",
                    'yellow'
                )
            )
