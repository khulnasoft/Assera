...
from assera import Package, Target, Array
from assera.hat import ONNXHATPackage
from assera.samples import MLAS, MLASOptions
...
def handle_gemm_node(node, model, package, target=Target.HOST):
    ...
    print(f"[assera] handle_gemm_node called for \nA = [{', '.join(map(str, A_shape))}]"
    ...
    # assera BUG: adding name to ctor throws
...
def handle_matmul_node(node, model, package, target=Target.HOST):
    ...
    print(f"[assera] handle_matmul_node called for \nA = [{', '.join(map(str, A_shape))}]"
          f"\nB = [{', '.join(map(str, B_shape))}]"
          f"\nC = [{', '.join(map(str, C_shape))}]"
