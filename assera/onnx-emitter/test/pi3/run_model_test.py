...
def assera_provider_settings(hat_package_dir):
...
def create_assera_settings_from_package(hat_package):
    settings = assera_provider_settings(hat_package)
    return {'assera_settings': json.dumps(settings)}
...
def test_transformer_model_on_target(output_dir):
    ...
    for base_name in model_base_name_list:
        for batch, seq in model_batch_sep_list:
            ...
            rc_time = test_model_on_target_assera(optimized_onnx_model, output_dir, options=options, num_results=num_results)
...
def test_model_on_target_assera(model, output_dir, outputs=None, options=None, num_results=10000, syms=None, large_model=True, skip_correctness=False, min_time_in_sec=15, num_batches=10):
    os.environ["OMP_NUM_THREADS"] = "1"
    ...
    assera_settings = create_assera_settings_from_package(output_dir)
    providers = [
        ('AsseraExecutionProvider', assera_settings),
        ('CPUExecutionProvider', {}),
    ]
    ...
    print(f"Running Assera EP session for {min_time_in_sec}s")
    iterations = 0
    wall_clock_start = perf_counter()
    wall_clock_end = perf_counter()
    batch_times = []
