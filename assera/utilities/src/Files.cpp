...
namespace utilities
{
    ...
    void EnsureDirectoryExists(const std::string& path)
    {
        if (!DirectoryExists(path))
        {
            ...
            if (rc != 0)
            {
                throw assera::utilities::Exception(assera::utilities::FormatString("mkdir failed with error code %d", errno));
            }
        }
    }
    ...
    std::string GetWorkingDirectory()
    {
        ...
        if (rc != 0)
        {
            throw assera::utilities::Exception(assera::utilities::FormatString("error getting current working directory: %d", rc));
        }
        return utf8wd;
    }
...
#pragma warning(disable : 4996)
        ...
        throw assera::utilities::Exception("Could not find '" + name + "' in your PATH environment");
    }
} // namespace utilities
} // namespace assera
