        ...
        MemoryShape StridedIncrement(const MemoryShape& extent, const MemoryShape& strides)
        {
            ...
            for (int64_t index = numDimensions - 1; index >= 0; --index)
            {
                if (extent[index] == mlir::ShapedType::kDynamicSize)
                {
                    ...
                    throw assera::utilities::InputException(assera::utilities::InputExceptionErrors::invalidArgument,
...
size_t std::hash<::assera::utilities::DimensionVector>::operator()(const ::assera::utilities::DimensionVector& v) const noexcept
{
    return ::assera::utilities::HashValue(v.ToVector());
}

std::size_t std::hash<::assera::utilities::MemoryLayout>::operator()(const assera::utilities::MemoryLayout& arg) const noexcept
{
    using ::assera::utilities::HashCombine;
