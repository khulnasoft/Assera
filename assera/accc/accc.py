...
class BuiltAsseraEmittedLibrary:
...
class AsseraCMakeProject:
    ...
    def build(
        ...
        if not quiet:
            print(
                f"\n### Building Assera cmake project for {self.target_name} in {build_dir} with config {build_config}...\n"
            )
        ...
        if self.target_type == self.TargetType.Executable:
            return BuiltAsseraProgram(built_target_path)
        else:
            return BuiltAsseraEmittedLibrary(built_target_path)
...
def deploy_assera_generator_project(
    deploy_dir,
    generator_name,
    dsl_src_filepath,
    ...
    return AsseraCMakeProject(
        deploy_dir, target_name=generator_name, target_type=AsseraCMakeProject.TargetType.Executable, pretend=pretend
    )


def deploy_assera_emitted_lib_project(
    deploy_dir,
    library_name,
    obj_files,
    additional_text_replacements={},
    ...
    return AsseraCMakeProject(
        deploy_dir, target_name=library_name, target_type=AsseraCMakeProject.TargetType.StaticLibrary, pretend=pretend
    )


def deploy_assera_main_project(
    deploy_dir,
    main_name,
    library_name,
    main_src_filepath,
    ...
    return AsseraCMakeProject(
        deploy_dir, target_name=main_name, target_type=AsseraCMakeProject.TargetType.Executable, pretend=pretend
    )
...
class ModuleFileSet:
    ...
    def __repr__(self):
        desc = [
            ...
            f"Assera MLIR: {self.generated_mlir_filepath}",
...
class AsseraProject:
    ...
    def __init__(
        ...
        if dsl_src_filepath:
            ...
            self.generator_project = deploy_assera_generator_project(
    ...
    def build_main(
        ...
        if self.print_subprocess_output:
            stdout = None
            stderr = None
        self.main_project = deploy_assera_main_project(
    ...
    def build_static_lib(
        ...
        self.emitted_lib_project = deploy_assera_emitted_lib_project(
...
def accc(
    ...
    # Create Assera generator parameters
