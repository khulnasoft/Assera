    ...
    if rocblas or cublas or pytorch:
        ...
        for gemm in data:
            ...
            for gpu_id in range(len(available_gpus)):
                ...
                benchmarkResult = assera_gemm.BenchmarkResult(opts=gemm, dtype=dtype, gpu_id=best_gpu, commit_id=commit_id, commit_datetime=commit_datetime, commit_branch=commit_branch, target_name=target_name, deviceProperties=deviceProperties[best_gpu])
        ...
        for gemm in data:
            ...
            ldc = gemm.m
            benchmarkResult = assera_gemm.BenchmarkResult(opts=gemm, dtype=dtype, gpu_id=0, commit_id=commit_id, commit_datetime=commit_datetime, commit_branch=commit_branch, target_name=target_name, deviceProperties=deviceProperties[0])
        ...
        for gemm in data:
            ...
            assera_gemm.benchmark_gemm(gemm, dtype, batch_size, output_prefix, category, available_gpus, container_name, verbose, compiler_ver, commit_id, commit_datetime, commit_branch, target_name, check, relu, deviceProperties)
        # else:
        #     if container_name:
