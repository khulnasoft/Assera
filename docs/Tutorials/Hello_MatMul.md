...
* Implement a simple Matrix Multiplication (MatMul) function using Assera's Domain Specific Language (DSL)
* Produce a [HAT](https://github.com/microsoft/hat) package containing the MatMul function
* Call the function from C or C++ code

### Prerequisites

* This tutorial assumes you already have Assera installed. If not, you can find the instructions in [here](../Install/README.md)
...
We will now walk through a naive Matrix Multiplication (MatMul) using Assera.

Create an empty file called `hello_matmul_generator.py`. First we'll import Assera's module.
...
#
# If k is not divisible by 4, Assera will take care of the boundary
# case for you.
kk = sched.split(k, 4)
```
...
By now, you should have all the code necessary to generate your first Assera MatMul function. You can also find the complete Python script [here](hello_matmul/hello_matmul_generator.py).
...
After this runs, you should see a header file `hello_matmul.hat` and some object files (such as `hello_matmul.obj` or `hello_matmul.o`). The `.hat` file format is described [here](https://github.com/microsoft/hat). In Assera, we call these files the "HAT package".
