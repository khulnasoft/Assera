...
## `assera.Package.build(name[, format, mode, platform, tolerance, output_dir])`
...
`format` | The format of the package. | `assera.Package.Format`, defaults to `Package.Format.HAT_STATIC`
...
`platform` | The platform where the package runs. | `assera.Package.Platform`
...
Build a package with error checking for `func1`, outputing error messages to `stderr` if the default implementation and the Assera implementation do not match within a tolerance of `1.0e-6`:
