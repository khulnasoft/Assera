...
Assera's parameters are placeholders that get replaced with concrete values when adding a function to a package. A parameter can be used in a `Nest`, a `Schedule`, or a `Plan`.
...
Assera's parameters support the basic arithmetic operations and other relational/bitwise/intrinsics operations. For example, we can add the following code snippet instead:
...
Assera provides an easy way to add all the functions that correspond to the parameter grid at once:
```python
